...
% Example:
% theta = [ a0 a1 b0 b1 b2 b3 c0 c1 c2 ]
% evalCn( [ 1 3 2 ], [ -4 0 ], theta, xx)
% where the first segment is a linear: a0 + a1 * x
...
function pval = evalCn(degree, breaks, theta, xx)
	pval = [];
	breaks = sort(breaks(:)); % ensure/convert is a vertical vector
